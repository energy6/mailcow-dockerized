...
jQuery(function($){
  ...
  function draw_auth_logs() {
    // just recalc width if instance already exists
    if ($.fn.DataTable.isDataTable('#auth_log') ) {
      $('#auth_log').DataTable().columns.adjust().responsive.recalc();
      return;
    }

    var table = $('#auth_log').DataTable({
      responsive: true,
      processing: true,
      serverSide: false,
      stateSave: true,
      ...
      order: [[0, 'desc']],
      initComplete: function(){
        hideTableExpandCollapseBtn('#tab-auth-logs', '#auth_log');
      },
      ajax: {
        type: "GET",
  ...
  onVisible("[id^=auth_log]", () => draw_auth_logs());
